...
<mapper namespace="${daoPackage}">
    ...
    <sql id="column_map">
        <#list fields as field>`${field.column}` AS ${field.humpName}<#if field_has_next>,</#if></#list>
    </sql>
    ...
    <select id="select" resultType="${modelPackage}">
        SELECT <include refid="column_map"/> FROM `${tableName}`
    </select>

    <select id="selectById" resultType="${modelPackage}">
